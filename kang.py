...
def validate_packname(packname: str) -> str:
    """
    Validates the sticker pack name, making sure it only contains lowercase letters, digits, and underscores.
    """
    return re.sub(r'[^a-z0-9_]', '', packname.lower())

# Function to create a new sticker pack if the set is invalid
def create_new_sticker_pack(msg, user, bot, packname, sticker_emoji, kangsticker):
    try:
        # Create a new sticker pack
        bot.create_new_sticker_set(
            user_id=user.id,
            name=packname,
            title=f"{user.first_name}'s Stickers",
            png_sticker=open(kangsticker, "rb"),
            emojis=sticker_emoji,
        )
        msg.reply_text(
            f"New sticker pack created successfully! [Add it here](t.me/addstickers/{packname})",
            parse_mode=ParseMode.MARKDOWN,
        )
    except TelegramError as e:
        msg.reply_text("Failed to create a new sticker pack.")
        logger.error(f"Error creating new sticker pack: {e}")

# Function for creating the sticker based on image and text, and show username at the top
@run_async
def kang_image(bot: Bot, update: Update):
    ...
    # Generate the sticker pack name based on user's name or username
    packname = f"{user.first_name}by{bot.username}" if user.first_name else f"{user.username}by{bot.username}" if user.username else f"a{user.id}by{bot.username}"
    ...
    background_color = generate_user_color(user.username if user.username else str(user.id))
    ...
    try:
        ...
        try:
            ...
            if "Stickerset_invalid" in e.message:
                ...
                create_new_sticker_pack(msg, user, bot, packname, sticker_emoji, kangsticker)
            else:
...
def validate_packname(packname: str) -> str:
    """
    Validate and sanitize the sticker pack name.
    Telegram allows only lowercase letters, digits, and underscores in pack names.
    """
...
def kang_message(bot: Bot, update: Update):
    ...
    # Generate the sticker pack name based on user's first name, username, or user ID
    if user.first_name:
        packname = f"{user.first_name}by{bot.username}"
    elif user.username:
        packname = f"{user.username}by{bot.username}"
    else:
        packname = f"a{user.id}by{bot.username}"
    
    packname = validate_packname(packname)  # Validate the sticker pack name
    sticker_emoji = "✍️"  # Default emoji for text sticker
...
# Function to create a sticker based on text message with a custom color
@run_async
def kang_text_color(bot: Bot, update: Update, args):
    ...
    # Handle custom color argument or default to 'black'
    color = args[0] if args else "black"
    
    # Generate the sticker pack name
    if user.first_name:
        packname = f"{user.first_name}by{bot.username}"
    elif user.username:
        packname = f"{user.username}by{bot.username}"
...
def makepack_internal(msg, user, sticker_file, sticker_emoji, bot, packname, tries):
    """
    A helper function to create a new sticker pack if the current one is invalid.
    """
    try:
        bot.create_new_sticker_set(
            user_id=user.id,
        ...
        msg.reply_text(f"New sticker pack created! [Add it here](t.me/addstickers/{packname})", parse_mode=ParseMode.MARKDOWN)
    except TelegramError as e:
        msg.reply_text("Failed to create a new sticker pack.")
        logger.error(f"Error creating new sticker pack: {e}")
